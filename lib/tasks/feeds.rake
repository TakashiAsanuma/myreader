        ...
        results.entries.first(5).each do |feed|
          begin
            ...
            if feed.published <= time
              Feed.create(site_id: site.id, channel_id: site.channel_id, default_flag: site.channel.default_flag, title: feed.title, url: feed.url, published_at: feed.published, summary: summary)
            end
